...
import {
  armazemDoJogo, ate, cenarioLongo, faltamDe, fsmDe, liberacoes, quieto, rodarAte, saidaDe, serfDoJogo,
} from './helpers/serf-cenario';
import {
  cenarioLigado, comArmazemCompleto, comEstoqueNaSaida, comEstradas, comObra, comUnidadeEm, inicial, linhaH, linhaV, semAUnidade,
...
function rodarCaos(semente: number, passos: number, cobertura: CoberturaDoCaos): void {
  ...
  for (let i = 0; i < passos; i++) {
    ...
    switch (acao) {
      ...
      default: break; // 0..3: so deixa o tempo passar
    }
